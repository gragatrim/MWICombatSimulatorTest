class CombatUtilities {
    ...
    static processAttack(source, target, abilityEffect = null) {
        ...
        if (CombatUtilities.deterministic ? hitChance > 0 : CombatUtilities.random() < hitChance) {
            ...
            if (CombatUtilities.deterministic && !source.isPlayer && target.isPlayer) {
                mitigatedDamage *= hitChance * hitChance;
            }
        ...
        if (target.combatDetails.combatStats.retaliation > 0) {
            ...
            if (CombatUtilities.deterministic ? retaliationHitChance > 0 : retaliationHitChance > CombatUtilities.random()) {
                ...
                if (CombatUtilities.deterministic && source.isPlayer) {
                    mitigatedRetaliationDamage *= retaliationHitChance * retaliationHitChance;
                }
